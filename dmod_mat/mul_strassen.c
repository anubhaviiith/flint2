...
void _dmod_mat_mul_strassen(dmod_mat_t C, const dmod_mat_t A, const dmod_mat_t B)
{
    ...
    _dmod_mat_window_init(A12, A, 0, anc, anr, 2*anc);
    _dmod_mat_window_init(A21, A, anr, 0, 2*anr, anc);
    _dmod_mat_window_init(A22, A, anr, anc, 2*anr, 2*anc);

    _dmod_mat_window_init(B11, B, 0, 0, bnr, bnc);
    _dmod_mat_window_init(B12, B, 0, bnc, bnr, 2*bnc);
    _dmod_mat_window_init(B21, B, bnr, 0, 2*bnr, bnc);
    _dmod_mat_window_init(B22, B, bnr, bnc, 2*bnr, 2*bnc);

    _dmod_mat_window_init(C11, C, 0, 0, anr, bnc);
    _dmod_mat_window_init(C12, C, 0, bnc, anr, 2*bnc);
    _dmod_mat_window_init(C21, C, anr, 0, 2*anr, bnc);
    _dmod_mat_window_init(C22, C, anr, bnc, 2*anr, 2*bnc);
    ...
    _dmod_mat_window_refresh(C11, C, 0, 0, anr, bnc);
    _dmod_mat_window_refresh(C12, C, 0, bnc, anr, 2*bnc);
    _dmod_mat_window_refresh(C21, C, anr, 0, 2*anr, bnc);
    _dmod_mat_window_refresh(C22, C, anr, bnc, 2*anr, 2*bnc);
    ...
    {
        dmod_mat_t Bc, Cc;
        _dmod_mat_window_init(Bc, B, 0, 2*bnc, b, c);
        _dmod_mat_window_init(Cc, C, 0, 2*bnc, a, c);
        _dmod_mat_mul(Cc, A, Bc);
        _dmod_mat_window_refresh(Cc, C, 0, 2*bnc, a, c);
    ...
    {
        dmod_mat_t Ar, Cr;
        _dmod_mat_window_init(Ar, A, 2*anr, 0, a, b);
        _dmod_mat_window_init(Cr, C, 2*anr, 0, a, c);
        _dmod_mat_mul(Cr, Ar, B);
        _dmod_mat_window_refresh(Cr, C, 2*anr, 0, a, c);
    ...
    if (b > 2*anc) 
    {
        ...
        _dmod_mat_window_init(Ac, A, 0, 2*anc, 2*anr, b);
        _dmod_mat_window_init(Br, B, 2*bnr, 0, b, 2*bnc);
        _dmod_mat_window_init(Cb, C, 0, 0, 2*anr, 2*bnc);
        
        _dmod_mat_init(temp, Ac->nrows, Br->ncols, A->mod);
        _dmod_mat_mul(temp, Ac, Br); 
        _dmod_mat_add(Cb, Cb, temp);
       
        _dmod_mat_window_refresh(Cb, C, 0, 0, 2*anr, 2*bnc);
